...
## 🛡️ 五、 執行安全與品質守則
1. **嚴格遵循 Conventional Commits**：每次階段性產出均以原子提交儲存，禁止將多個 Phase 的變更混在同一個 Commit。
2. **每次變更必須經過 `verify` 驗證**：重構期間確保現有測試不中斷。
3. **持續更新 MEMORY.md**：每個會話結束前更新當前進度與踩坑紀錄，確保跨工作階段無縫接軌。
